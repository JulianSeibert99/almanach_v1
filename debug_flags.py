...
def find_missing():
    tree = ET.parse("staaten-bpb-utf8.xml")
    root = tree.getroot()
    
    for country in root.findall(".//staat"):
        name = country.find("hname").text
        autokennz = country.find("autokennz").text or ""
        iso3 = ""
        if "|" in autokennz:
        ...
        if iso3 not in ISO3_TO_ISO2:
            print(f"Missing: {name} (ISO3: {iso3})")
